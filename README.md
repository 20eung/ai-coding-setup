...
**MacOS, Linux, Windows** 환경에서 터미널만 익숙하면 누구나 따라할 수 있는 AI 코딩 환경 설정 가이드입니다.
Antigravity로 20분 만에 첫 AI 코딩을 체험하고, 필요에 따라 Claude Code, Gemini CLI, n8n 자동화까지 단계적으로 확장할 수 있습니다.
...
- 🚀 **쉬운 시작 (PART 1)**: Antigravity로 약 20분 만에 첫 AI 코딩 체험 — API 키 없이 내장 AI 모델 즉시 사용
- 🧠 **AI 도구 확장 (PART 2)**: Gemini CLI, Claude Code, GitHub 자동화로 개발 생산성 극대화
- 🔄 **워크플로우 자동화 (PART 3)**: Docker + n8n + Claude Code로 Python 서비스를 GUI 자동화로 전환
- 🛡️ **기업용 보안 연동**: AWS Bedrock, Azure AI Foundry, GCP Vertex AI IAM 인증 지원
- 🔌 **bkit + n8n-MCP**: AI 네이티브 개발 방법론(PDCA)과 n8n 1,200+ 노드 문서 자동 주입

## ⚡ Quick Start — 지금 바로 시작!

**AI 코딩을 처음 시작하는 분은 PART 1부터 시작하세요. 약 20분이면 AI와 첫 대화가 가능합니다.**

**[→ PART 1: 첫 AI 코딩으로 바로 이동](#part-1-첫-ai-코딩-모두-대상--약-20분)**

> 💡 Claude Code, Docker, n8n은 필요할 때 PART 2-3에서 설정하세요. PART 1만으로도 충분히 AI 코딩을 체험할 수 있습니다.
...
3. [Antigravity 설치 및 설정](#3-antigravity-설치-및-설정)
   - [3.1 설치](#31-설치)
   - [3.2 확장 프로그램](#32-확장-프로그램)
   - [3.3 리모트 서버 접속방법](#33-리모트-서버-접속방법)

### PART 2: AI 코딩 환경 완성 (심화)

4. [Gemini CLI 설치 및 설정](#4-gemini-cli-설치-및-설정)
   - [4.1 설치](#41-설치)
   - [4.2 API 설정](#42-api-설정)
5. [Claude Code 설치 및 설정](#5-claude-code-설치-및-설정)
   - [5.1 설치](#51-설치)
   - [5.2 클라우드 AI 서비스 연동 (AWS / Azure / GCP)](#52-클라우드-ai-서비스-연동-aws--azure--gcp)
   - [5.3 프로젝트 컨텍스트 설정 (CLAUDE.md)](#53-프로젝트-컨텍스트-설정-claudemd)
   - [5.4 터미널 활용법](#54-터미널-활용법-대화형-cli)
...
10. [n8n-MCP + n8n-skills 설정](#10-n8n-mcp--n8n-skills-설정)
    ...
    - [10.2 n8n-skills란?](#102-n8n-skills란)
    - [10.3 설치 및 연동](#103-설치-및-연동)
...
## PART 1: 첫 AI 코딩 (모두 대상) — 약 20분

> **이 파트만 완료해도 AI와 대화하며 코딩을 도움받을 수 있습니다.**
> Claude Code, Docker, n8n은 나중에 필요할 때 PART 2-3에서 설정하세요.
...
- **Python**: [Python 공식 홈페이지](https://www.python.org/)에서 3.10 이상의 버전을 권장합니다. (PART 2부터 필요)
- **Git**: [Git 공식 홈페이지](https://git-scm.com/)에서 설치 가능합니다.
- **GitHub CLI (gh)**: [GitHub CLI 공식 홈페이지](https://cli.github.com/)에서 설치 가능합니다. AI 에이전트의 PR/릴리즈 자동화에 필수입니다. (PART 2, 6.1절 참고)
...
GitHub 또는 리모트 서버 접속을 편리하게 하기 위해 파일을 생성하거나 수정합니다.
...
SSH Config에 등록한 리모트 서버(`remote_server_alias`)에 비밀번호 없이 접속하려면, 로컬의 공개키를 서버에 등록해야 합니다.
...
## 3. Antigravity 설치 및 설정

### 3.1 설치

Antigravity는 강력한 AI 코딩 에이전트 환경입니다. 별도의 설치 과정 없이 지원되는 환경에서 즉시 사용 가능하거나, 전용 클라이언트를 통해 구동됩니다.

[Antigravity 홈페이지](https://antigravity.google/) 에서 다운로드 및 설치 방법을 확인하세요.

![Antigravity 첫 실행 화면](docs/assets/antigravity.png)

### 3.2 확장 프로그램

Antigravity와 함께 사용하면 효과가 극대화됩니다.

| 확장 프로그램               | 마켓플레이스 ID                     | 용도                                     |
| --------------------------- | ----------------------------------- | ---------------------------------------- |
| **Claude Code for VS Code** | `anthropic.claude-code`             | Antigravity 내에서 Claude 기능 직접 사용 |
| **Korean Language Pack**    | `ms-ceintl.vscode-language-pack-ko` | 한국어 메뉴 구성                         |

확장 마켓플레이스에서 위 ID로 검색하여 설치할 수 있습니다.

![확장 프로그램 설치 화면](docs/assets/antigravity-extension.png)

### 3.3 리모트 서버 접속방법

Antigravity에 내장된 **Remote-SSH** 기능을 사용하면 GUI 환경에서 리모트 서버에 간편하게 접속할 수 있습니다.

1. **설정 연동**: Remote-SSH는 이전에 작성한 `~/.ssh/config` 파일을 자동으로 읽어와 목록을 표시합니다.
2. **접속 방법**: 왼쪽 패널의 '원격 탐색기' 아이콘을 클릭하면 등록된 서버 목록(`remote_server_alias` 등)이 나타납니다. 접속하려는 서버 옆의 '새 창에서 연결' 아이콘을 클릭합니다.
3. **효과**: 터미널 명령어 없이 클릭만으로 원격 서버의 파일 구조를 탐색하고 직접 편집할 수 있습니다.

---

> ✅ **PART 1 체크포인트**: Antigravity에서 AI와 대화가 되면 완료입니다!
> 더 이상 설정하지 않아도 됩니다. 심화 기능이 필요할 때 아래 PART 2로 이동하세요.

---

## PART 2: AI 코딩 환경 완성 (심화)

> Claude Code, Gemini CLI 등 독립적인 AI 도구를 완전히 활용하고 싶은 분들을 위한 설정입니다.
...
3. 새 PowerShell 창을 열거나 아래 명령으로 프로필을 다시 로드합니다:
   ```powershell
   . $PROFILE
   ```
4. 이후 터미널에서 아래 명령으로 키를 전환합니다:
   ```powershell
   Set-GeminiWork      # 업무용 키로 전환
   Set-GeminiPersonal  # 개인용 키로 전환
   ```
...
Anthropic의 코딩 어시스턴트인 Claude Code를 설치합니다.
...
### 5.2 클라우드 AI 서비스 연동 (AWS / Azure / GCP)

> [!NOTE]
> AWS Bedrock, Azure AI Foundry, GCP Vertex AI는 모두 **완전관리형 AI 서비스**로,
> 기업용 IAM 인증을 통해 API 키 없이 Claude를 사용할 수 있습니다.
> 보안 강화, 비용 통합, 규정 준수가 중요한 **팀/기업 환경**에 적합합니다.
> 개인 사용자나 빠른 테스트 목적이라면 [Anthropic 직접 API](https://console.anthropic.com/)가 더 간편합니다.

| 클라우드 | 서비스명   | Claude Code 환경변수        | 공식 문서                                                                    |
| -------- | ---------- | --------------------------- | ---------------------------------------------------------------------------- |
| AWS      | Bedrock    | `CLAUDE_CODE_USE_BEDROCK=1` | [claude-code-use-bedrock](https://github.com/20eung/claude-code-use-bedrock) |
| Azure    | AI Foundry | `CLAUDE_CODE_USE_AZURE=1`   | [Azure AI Foundry](https://azure.microsoft.com/en-us/products/ai-foundry/)   |
| GCP      | Vertex AI  | `CLAUDE_CODE_USE_VERTEX=1`  | [Vertex AI](https://cloud.google.com/vertex-ai)                              |

아래는 **AWS Bedrock** 기준 설정 방법입니다. Azure/GCP는 각 공식 문서를 참고하세요.
...
#   일반 리전:            anthropic.claude-sonnet-4-6
#   Cross-Region (미국):  us.anthropic.claude-sonnet-4-6
...
# setx ANTHROPIC_MODEL "us.anthropic.claude-sonnet-4-6-20251001-v1:0"
```

### 5.3 프로젝트 컨텍스트 설정 (CLAUDE.md)
...
![Antigravity 터미널에서 Claude Code 실행 화면](docs/assets/claude-code.png)
...
3.2에서 설치한 **Claude Code for VS Code** 확장 프로그램을 활용하는 방법입니다.
...
![Claude Code for VS Code 확장 프로그램 실행 화면](docs/assets/claude-code-for-vscode.png)
...
> [!NOTE]
> `/plugin`은 Claude Code의 플러그인 마켓플레이스 커맨드입니다. bkit은 이 명령어를 통해 설치됩니다.
...
> Claude Code와 n8n을 연동하여 워크플로우 자동화를 구현합니다. Python 서비스를 n8n으로 마이그레이션하거나, AI가 직접 워크플로우를 생성/배포하는 환경을 구축합니다.
...
아래 내용을 Claude Code에 붙여넣어 실행하세요.

```
아래 링크를 참고해서 n8n-MCP와 n8n-skills를 설치하고 Claude Code와 연동시켜줘.
